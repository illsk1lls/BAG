...
ECHO EMPTYING BAG...
powershell -nop -c $file=Get-Content '%~f0'; $r=[regex]::Match^($file,'\:\:^([^^^\:]+^)\:\:^(.+?^)\:\:\1\:\:'^).Groups[2].Value.Replace^(' ',"""`n"""^).Replace^('::',''^) ^| Set-Content '%TEMP%\%~n0.000' >nul
powershell -nop -c ^(Get-Content '%~f0' ^| Select-Object -Skip 36^) ^| Set-Content '%TEMP%\%~n0.001' >nul
SET /p FNAME=<"%TEMP%\%~n0.001" 
DEL "%TEMP%\%~n0.001" /F /Q>nul
SETLOCAL ENABLEDELAYEDEXPANSION
CERTUTIL -DECODE -F "%TEMP%\%~n0.000" "%~dp0!FNAME::=!" >nul
ENDLOCAL
DEL "%TEMP%\%~n0.000" /F /Q>nul
