...
EXIT /b
:EMPTYBAG
IF %~z0 LSS 3087 ECHO The BAG is already empty, drag-and-drop something onto the BAG to put it inside. ;^) & ECHO. & PAUSE & EXIT /b
IF %~z0 GEQ 80000000 (ECHO EMPTYING BAG... ^(This may take a while^)) ELSE (ECHO EMPTYING BAG...)
>nul 2>&1 POWERSHELL -nop -c "$f=GC '%~f0'; $m=[regex]::Matches($f,'::([^^:]+)::(.+?)::\1::') | Select-Object -Skip 1 | %% {$n1=$_.Groups[1].Value; $n2=[System.Text.Encoding]::Utf8.GetString([System.Convert]::FromBase64String("^""$n1"^"")); if ($n2 -match '^DUPLICATE -(\d+)-') {$n = [int]$Matches[2]}; $fn=$n2; while(Test-Path -LiteralPath "^""%~dp0$fn"^"") {$n++; if ($fn -match '^DUPLICATE -(\d+)-') {$fn = $fn -Replace '^DUPLICATE -(\d+)-',"^""DUPLICATE -$n-"^""}else{$fn="^""DUPLICATE -$n- $n2"^""}}; $d=$_.Groups[2].Value; $i=[System.IO.MemoryStream]::new([Convert]::FromBase64String($d)); $o=(New-Item "^""$fn"^"" -Force).OpenWrite(); $g=[System.IO.Compression.GZipStream]::new($i,[System.IO.Compression.CompressionMode]::Decompress); $g.CopyTo($o); $g.Dispose(); $o.Dispose(); $i.Dispose(); $n=0}; (GC '%~f0' -TotalCount 21) | SC '%~f0'"
EXIT /b